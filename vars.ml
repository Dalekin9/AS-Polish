...
let vars_polish (p:program) : unit =
  ...
  let rec vars (p:program) ((init: Names.t) ,(non_init: Names.t)) (pos: position) =

    let analyse_instr (instr) ((init: Names.t) ,(non_init: Names.t)) =
      let rec analyse_expr (expr) ((init: Names.t) ,(non_init: Names.t)) =         
        match expr with
        |Num(x) -> (init,non_init)
      ...
      and analyse_cond (cond) ((init: Names.t), (non_init: Names.t)) =
        match cond with
